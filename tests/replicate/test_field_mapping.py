...
def test_word_status_passthrough_from_pg():
    """status must pass through from PG row, not be hardcoded."""
    for status_val in (0, 1, 2):
        ...
        assert row["status"] == status_val, f"expected status={status_val}, got {row['status']}"
